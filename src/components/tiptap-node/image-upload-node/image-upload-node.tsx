  ...
  const handleUpload = async (files: File[]) => {
    ...
    if (urls.length > 0) {
      ...
      if (isValidPosition(pos)) {
        ...
        // Move cursor to next node and insert a fresh upload node below
        focusNextNode(props.editor)
        // Insert a new upload node so a subsequent image goes below the previous one
        ;(props.editor as any).commands.setImageUploadNode({
          type: extension.options.type,
          accept: extension.options.accept,
          limit: extension.options.limit,
          maxSize: extension.options.maxSize,
          upload: extension.options.upload,
          onError: extension.options.onError,
          onSuccess: extension.options.onSuccess,
          HTMLAttributes: extension.options.HTMLAttributes,
        })
      }
    }
  }
