...
/**
 * Uploads an image to the Next.js API which stores it (e.g., Supabase) and returns a public URL.
 * Uses FileReader progress events to report client-side read progress; network progress depends on fetch.
 */
export async function uploadImageToServer(
  ...
  // Read file as base64 with progress if available
  ...
  const res = await fetch("/api/upload-image", {
    ...
    body: JSON.stringify({ noteId, fileBase64: base64, fileName: file.name }),
    signal: abortSignal,
  })
